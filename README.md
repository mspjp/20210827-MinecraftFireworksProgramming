...
マイクラ上でキーボードの `/` キーを押すとウィンドウをアクティブにしたまま別のウィンドウ操作ができるようになります。マイクラの操作に戻る場合は `ESC` を押します。初回起動の際には全画面表示になっていますので、`F11` キーを押して全画面モードを解除してください。
...
# 3. 花火を打ち上げる
マイクラで花火を打ち上げるために必要なパッケージ（ライブラリ）を導入します。今回使用するライブラリは [MinecraftConnection](https://github.com/takunology/MinecraftConnection) です。

PCのデスクトップ上で `function.proj` というファイルを作成してください。（ファイル名が正しくないとパッケージを導入出来ません。）

Windowsをお使いの方はデスクトップ上で右クリック→新規作成→テキストドキュメントを選択して、ファイル名を上記のように変更します。拡張子が表示されない場合はエクスプローラーを開き、表示タブから `ファイル名拡張子` のチェックボックスにチェックを入れてください。
...
まずは、関数アプリを実行してマイクラで花火を打ち上げるための手順を説明します。下記のコードをコピーして貼り付けて貼り付けてください。（サーバアドレスとパスワードはお伝えしたものを使用してください。）
...
https://forms.office.com/r/s5z26VjFtb
